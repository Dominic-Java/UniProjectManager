...
<head>
    <meta charset="utf-8">
    <title>Avertizare restanta</title>
</head>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #f0d7c2;border-radius:12px;">
    <tr>
        <td style="padding:24px;">
            <h1 style="margin:0 0 12px;font-size:24px;">Avertizare restanta</h1>

            <p style="margin:0 0 12px;line-height:1.5;">
                Salut, <strong>{{ $grade->student?->first_name ?? $grade->student?->name ?? 'student' }}</strong>!
                Ai o nota sub 5, deci materia este in status de restanta.
            </p>
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:14px 0;border:1px solid #f0d7c2;border-radius:10px;">
                <tr>
                    <td style="padding:12px;">
                        ...
                        <p style="margin:0 0 8px;"><strong>Profesor:</strong> {{ $grade->classroom?->createdBy?->name ?? '-' }}</p>
                        <p style="margin:0;"><strong>Ultima actualizare:</strong> {{ $grade->updated_at?->format('d.m.Y H:i') ?? now()->format('d.m.Y H:i') }}</p>
                    </td>
                </tr>
            </table>
